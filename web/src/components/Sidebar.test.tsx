...
describe("Sidebar", () => {
  ...
  it("clicking away from a blank New Project input dismisses it instead of creating anything", async () => {
    mswServer.use(http.get("/api/columns/root", () => HttpResponse.json([AREA])));
    let called = false;
    mswServer.use(
      http.post("/api/commands", () => {
  ...
  it("clicking away from a New Project input with a title creates the project", async () => {
    mswServer.use(http.get("/api/columns/root", () => HttpResponse.json([AREA])));
    let capturedBody: unknown;
    mswServer.use(
      http.post("/api/commands", async ({ request }) => {
