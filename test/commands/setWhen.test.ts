...
describe("SetWhen.apply", () => {
  it("sets when_date and stamps updated_at", () => {
    ...
    new SetWhen(node.id, "2024-07-01").apply(ctx);
  ...
  it("accepts null to clear when_date", () => {
    const node = newNodeInput({ type: "todo", whenDate: "2024-07-01" });
    repo.insert(node);

    new SetWhen(node.id, null).apply(ctx);

    expect(repo.getById(node.id)?.whenDate).toBeNull();
  });

  it("is allowed on projects and on the Inbox", () => {
    new SetWhen(INBOX_ID, "2024-07-01").apply(ctx);
    expect(repo.getById(INBOX_ID)?.whenDate).toBe("2024-07-01");
  });
  ...
  it("rejects headings, which have no dates", () => {
    ...
    expect(() => new SetWhen(heading.id, "2024-07-01").apply(ctx)).toThrow(/heading/i);
  });

  it("throws if the node does not exist", () => {
    expect(() => new SetWhen("missing", "2024-07-01").apply(ctx)).toThrow(/not found/i);
  });
});

describe("SetWhen.invert", () => {
  it("restores the exact prior when_date and updated_at", () => {
    const node = newNodeInput({
      type: "todo",
      whenDate: "2024-05-01",
      updatedAt: "2024-01-01T00:00:00.000Z",
    ...
    repo.insert(node);
    const command = new SetWhen(node.id, "2024-07-01");

    command.apply(ctx);
    command.invert().apply(ctx);

    const row = repo.getById(node.id);
    expect(row?.whenDate).toBe("2024-05-01");
    expect(row?.updatedAt).toBe("2024-01-01T00:00:00.000Z");
  });

  it("throws if invert() is called before apply()", () => {
    expect(() => new SetWhen("some-id", "2024-07-01").invert()).toThrow(/apply/i);
  });
});
